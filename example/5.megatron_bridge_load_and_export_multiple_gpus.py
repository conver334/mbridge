#!/usr/bin/env python3
# Example showing how to use the unified AutoBridge wrapper
# 
# Single GPU usage:
#   python example_using_unified_wrapper.py --model_path /path/to/model
#
# Multi-GPU usage with TP/PP:
#   torchrun --nproc_per_node=8 example_using_unified_wrapper.py \
#       --model_path /path/to/model --tp 2 --pp 2
...
from mbridge.core.megatron_bridge_wrapper import AutoBridge
...
def main():
    ...
    # model = bridge.get_model(post_model_creation_callbacks=[freeze_moe_router, make_value_model])
    print(
    ...
    keys = bridge._bridge.hf_pretrained.state.keys()
    loaded_keys = set()
    not_matched_keys = set()
    for k, v in bridge.export_weights(model):
        if torch.distributed.get_rank() != 0:
            continue
        gt = bridge._bridge.hf_pretrained.state[k].cuda()
        if k != "lm_head.weight":
